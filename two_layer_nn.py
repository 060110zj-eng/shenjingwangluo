"""
=============================================================================
  基于 NumPy 的两层全连接神经网络 — 防过拟合增强版
  =====================================================================
  网络结构: 输入层 → 隐藏层(Sigmoid) → 输出层(Softmax)
  优化手段: 数据增强 + Mini-batch SGD + 动量 + L2正则 + 早停 + 学习率衰减
  作者: AI Assistant
  日期: 2026-08-06
=============================================================================
...
def load_images_from_folders(root_dir, target_size=(28, 28), grayscale=True,
    ...
    if verbose:
        ...
        print(f"\n✅ 加载完成: {X.shape[0]} 张图片, 每张 {h}×{w}×{channels} = {X.shape[1]} 特征")
...
X, y, CLASS_NAMES = load_images_from_folders(
    ...
    target_size=(32, 32),
    grayscale=False,   # 彩色图：保留颜色信息（红消防车、白急救包等有区分力）
)


# ===========================================================================
#  二、数据增强模块（核心：每个样本生成多个变体，扩大训练集）
# ===========================================================================
def augment_data(X, y, factor=4, noise_std=0.005, flip_prob=0.5, seed=42):
    ...
    增强策略:
      ...
      2. 随机翻转 — 模拟左右镜像（物体左右对称时有效）
      3. 像素值缩放 — 模拟亮度变化
    
    参数:
        X:         原始数据, shape (n, d)
        y:         原始标签, shape (n, c)
        factor:    增强倍数，factor=4 表示每个样本生成 4 个变体
        noise_std: 高斯噪声标准差（相对于像素值 0~1）
        flip_prob: 翻转概率（左右翻转）
        seed:      随机种子
...
# --- 防过拟合武器 ---
L2_LAMBDA     = 0.0001   # L2正则系数（轻微正则）
MOMENTUM      = 0.9      # 动量系数
BATCH_SIZE    = 0        # 0=全量梯度下降
LR_DECAY      = 0.0      # 0=不衰减
AUGMENT_FACTOR = 1       # 1=不增强
EARLY_STOP_PATIENCE = 400  # 早停：验证损失400轮不降则停，保留最佳模型
...
def cross_entropy_softmax_derivative(y_pred, y_true):
    """Softmax + 交叉熵的反向传播梯度: dL/dZ = y_pred - y_true"""
...
# ===========================================================================
#  六、两层全连接神经网络（Mini-batch SGD + 动量 + L2 + 早停）
# ===========================================================================
class TwoLayerNN:
    def __init__(self, n_input, n_hidden, n_output, lr=0.05, seed=42,
        ...
        np.random.seed(seed)
        self.lr = lr
        self.initial_lr = lr        # 保存初始学习率，用于重置
    ...
    def backward(self, X, y):
        ...
        # L2正则梯度在 update_params 中统一应用，这里只计算数据损失梯度
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs, batch_size=32,
        ...
        best_epoch = 0
        patience_counter = 0
        warmup_epochs = 200  # 前200轮不触发早停，让网络先学起来
...
if __name__ == "__main__":
    t_start = time.time()
    print("=" * 65)
    print("  基于 NumPy 的两层全连接神经网络（防过拟合增强版）")
    print("=" * 65)
    ...
    print(f"   L2={L2_LAMBDA} | Batch={BATCH_SIZE} | 增强×{AUGMENT_FACTOR} | 早停={EARLY_STOP_PATIENCE}轮")
    ...
    for i in range(show_num):
        ...
        flag = "✅" if test_pred_class[i] == test_true_class[i] else "❌"
        print(f"  {tn:<14} {pn:<14} {flag:<6}")
    
    print(f"\n{'='*65}")
    print(f"💡 当前配置:")
    print(f"   ① 彩色图 32×32 — 保留颜色区分信息")
    print(f"   ② 隐藏层 {N_HIDDEN}神经元 — 平衡拟合与泛化")
    print(f"   ③ 动量 {MOMENTUM} — 加速收敛、抑制震荡")
    print(f"   ④ L2正则 λ={L2_LAMBDA} — 轻微惩罚大权重")
    print(f"   ⑤ 全量梯度下降 — 稳定梯度方向")
    print(f"   ⑥ 固定学习率 {LEARNING_RATE} — 避免过早衰减")
    print(f"{'='*65}")