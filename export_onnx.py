...
def benchmark(session, name, label, warmup=20, runs=100):
    ...
    print(f"   {label:12s}  avg={avg:.1f}ms  med={med:.1f}ms  p99={p99:.1f}ms  min={np.min(times):.1f}ms  max={np.max(times):.1f}ms")
    return avg, med, p99
...
if os.path.exists(ONNX_INT8) and ONNX_INT8 != ONNX_FP32:
    try:
        ...
        benchmark(sess, 'INT8', "INT8 quantized")
    except Exception as e:
        print(f"   ❌ INT8: {e}")
...
benchmark(sess, 'FP32', "FP32 (1 thread)")
